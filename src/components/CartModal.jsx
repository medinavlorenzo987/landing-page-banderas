...
export default function CartModal({ cart, onClose, onConfirm, onOpenLegal, onUpdateQuantity, onRemoveItem }) {
    ...
    const validate = () => {
        ...
        if (esEmpresa) {
            ...
            } else if (!['10', '20'].includes(form.ruc.slice(0, 2))) {
                e.ruc = 'RUC inválido (debe empezar con 10 o 20)';
            }
        } else {
    ...
    const handleSubmit = async (e) => {
        ...
        setSubmitting(false);
        if (error) {
            setSubmitError('Hubo un problema al registrar tu pedido. Por favor intenta de nuevo.');
            return;
        }
        onConfirm(form);
    };
                ...
                {isEmpty ? (
                    ...
                    <>
                        ...
                        <form className="modal-form" onSubmit={handleSubmit} noValidate>
                            ...
                            {/* ── ERROR DE ENVÍO ── */}
                            {submitError && (
                                <p className="field-error" role="alert" style={{ marginBottom: '8px', textAlign: 'center' }}>
                                    {submitError}
                                </p>
                            )}
                            ...
                            <button
                                type="submit"
                                id="modal-submit-btn"
                                className={`modal-submit ${(!termsAccepted || submitting) ? 'modal-submit--disabled' : ''}`}
                                disabled={!termsAccepted || submitting}
                                aria-disabled={!termsAccepted || submitting}
                            ...
                            >
                                ...
                                {submitting ? 'Registrando pedido...' : 'Confirmar por WhatsApp'}
                            </button>
                        </form>
                    </>
                )}
